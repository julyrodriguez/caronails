...
exports.checkReminders = (0, scheduler_1.onSchedule)("every 10 minutes", async (event) => {
    ...
    try {
        ...
        const snapshot = await appointmentsRef
            .where("startAt", ">=", firestore_1.Timestamp.fromDate(minTime))
            .where("startAt", "<=", firestore_1.Timestamp.fromDate(maxTime))
            .get();
        if (snapshot.empty) {
        ...
        const upcomingAppointments = snapshot.docs
            ...
            .filter((app) => app.webReminderSent !== true && app.canceled !== true);
